...
async function aoReceberMensagem(slug, payload) {
  ...
  console.log(`[${slug}] aoReceberMensagem jid=${jid} tipo=${tipo}`);
  ...
  enfileirar(`${slug}:${jid}`, async () => {
    ...
    // Comando do dono
    console.log(`[${slug}] DEBUG numeroCliente=${numeroCliente} donoFone=${lavanderia.donoFone}`);
    if (numeroCliente === lavanderia.donoFone) {
      console.log(`[${slug}] DEBUG eh dono, checando comando`);
      const tratado = await comandosDoDono(lavanderia, textoFinal, slug);
      console.log(`[${slug}] DEBUG comandosDoDono retornou ${tratado}`);
      if (tratado) return;
    }
    ...
    console.log(`[${slug}] status atual da conversa ${conversa.id}: ${conversa.status}`);
    if (conversa.status === "HUMANO") {
      console.log(`[${slug}] conversa em HUMANO, bot nao responde`);
      return;
    }
    ...
    console.log(`[${slug}] DEBUG iniciando fluxo IA`);
    const historico = await prisma.mensagem.findMany({
      where: { conversaId: conversa.id },
      orderBy: { criadaEm: "asc" },
      take: 40,
    });
    console.log(`[${slug}] DEBUG historico carregado: ${historico.length} mensagens`);
    ...
    try {
      ...
      console.log(`[${slug}] DEBUG gerarResposta OK: resposta=${resposta?.slice(0,60)} precisaHumano=${precisaHumano}`);
    } catch (e) {
    ...
    if (!resposta) {
      console.log(`[${slug}] DEBUG resposta vazia sem precisaHumano, abortando`);
      return;
    }
    ...
    try {
      ...
      console.log(`[${slug}] DEBUG mensagem enviada ao cliente`);
    } catch (e) {
      console.log(`[${slug}] ERRO enviarMensagem: ${e.message}`);
    }
    const msgBot = await prisma.mensagem.create({ data: { conversaId: conversa.id, autor: "BOT", texto: resposta } });
    console.log(`[${slug}] DEBUG msgBot salva id=${msgBot.id}`);
    app.notificarWs(lavanderia.id, { tipo: "NOVA_MENSAGEM", conversaId: conversa.id, mensagem: msgBot });
  });
}
