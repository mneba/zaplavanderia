...
app.register(async (fastify) => {
  fastify.get("/painel/ws", { websocket: true }, async (socket, req) => {
    try {
      ...
      // Entrega QR imediatamente se houver um pendente
      const lav = await prisma.lavanderia.findUnique({ where: { id: lavanderiaId }, select: { slug: true } });
      if (lav) {
        const qrPendente = ultimoQr(lav.slug);
        if (qrPendente) socket.send(JSON.stringify({ tipo: "QR_ATUALIZADO", qr: qrPendente }));
      }
